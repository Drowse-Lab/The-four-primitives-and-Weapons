 ...
 * NBTタグで氷属性を持つ
 */
public class IceTestKatanaItem extends SwordItem {
    public IceTestKatanaItem() {
        super(new Tier() {
            public int getUses() {
                return 1000;
            }
    ...
    /**
     * アイテムに属性が設定されていることを確認し、なければ設定する
     */
    private void ensureElement(ItemStack stack) {
        if (!ElementalDamageUtils.hasElement(stack)) {
            ElementalDamageUtils.setElement(stack, ElementType.ICE, 2);
        }
    }
