...
public class OutpostSiteFeature extends Feature<NoneFeatureConfiguration> {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("the_four_primitives_and_weapons", "zennsyoukitiato"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 500) {
			...
			for (int a = 0; a < count; a++) {
				...
				BlockPos spawnTo = new BlockPos((int) (i + 0), (int) (j + 0), (int) (k + 0));
