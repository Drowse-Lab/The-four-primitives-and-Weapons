...
/**
 * 英雄級エンティティのレンダラー（ティア4）
 *
 * プレイヤーモデル（Steve/Slim）を使用し、武器と防具を表示します
 */
public class HeroicTierRenderer extends HumanoidMobRenderer<HeroicTierEntity, PlayerModel<HeroicTierEntity>> {

    private final PlayerModel<HeroicTierEntity> normalModel;  // Wide（太い腕）
    private final PlayerModel<HeroicTierEntity> slimModel;    // Slim（細い腕）
    ...
    public HeroicTierRenderer(EntityRendererProvider.Context context) {
    ...
    private static void initializeSkins() {
        try {
            ...
            System.out.println("[HeroicTier] Initialized skins: " + wideSkins.size() + " wide, " + slimSkins.size() + " slim");
        } catch (Exception e) {
            System.err.println("[HeroicTier] Error initializing skins: " + e.getMessage());
        }
    }

    @Override
    public void render(HeroicTierEntity entity, float entityYaw, float partialTicks,
    ...
    @Override
    protected void scale(HeroicTierEntity entity, PoseStack poseStack, float partialTicks) {
    ...
    private boolean shouldUseSlimModel(HeroicTierEntity entity) {
    ...
    private int getSkinIndex(HeroicTierEntity entity, boolean isSlim) {
    ...
    @Override
    public ResourceLocation getTextureLocation(HeroicTierEntity entity) {
        boolean isSlim = shouldUseSlimModel(entity);
