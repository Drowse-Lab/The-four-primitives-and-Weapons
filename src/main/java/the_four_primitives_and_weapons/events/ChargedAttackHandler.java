...
import the_four_primitives_and_weapons.entity.LokiDecoydasuEntity;
import the_four_primitives_and_weapons.entity.LokiDisarmEntity;
    ...
    private static void handleLokiTheTrickster(Player player, ChargeData data, boolean isLeftClickHeld) {
        ...
        // 左クリックを離した: 発動
            ...
            if (mode.equals("decoy") && data.lokiChargeTime >= DECOY_CHARGE_TIME) {
                TheFourPrimitivesAndWeaponsMod.PACKET_HANDLER.sendToServer(new AttackPacket(3, 0)); // attackType=3: Loki Decoy
            }
            // Disarmモード発動 - サーバーにパケット送信
            else if (mode.equals("disarm") && data.lokiChargeTime >= DISARM_CHARGE_TIME) {
                TheFourPrimitivesAndWeaponsMod.PACKET_HANDLER.sendToServer(new AttackPacket(4, 0)); // attackType=4: Loki Disarm
            }
