...
public class SheathEffectHandler {
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        boolean hasOldKatanaInHand = player.getMainHandItem().getItem() == TheFourPrimitivesAndWeaponsModItems.OLD_KATANA.get() ||
                                      player.getOffhandItem().getItem() == TheFourPrimitivesAndWeaponsModItems.OLD_KATANA.get();
        ...
        for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
            ...
            // 鞘アイテムかチェック
            if (stack.getItem() == TheFourPrimitivesAndWeaponsModItems.SAYA.get()) {
                if (stack.hasTag()) {
        ...
        if (!hasOldKatanaSheathed && offhandStack.getItem() == TheFourPrimitivesAndWeaponsModItems.SAYA.get()) {
        ...
        if (hasOldKatanaSheathed) {
            player.addEffect(new MobEffectInstance(
                TheFourPrimitivesAndWeaponsModMobEffects.SLICE_GUARD.get(), 
                60, // 3秒間（余裕を持たせる）
                1,  // レベル2（元のレベルと同じ）
                true, 
                false
