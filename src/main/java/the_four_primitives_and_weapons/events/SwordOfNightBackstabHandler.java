...
public class SwordOfNightBackstabHandler {
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        ...
        if (!player.hasEffect(TheFourPrimitivesAndWeaponsModMobEffects.SWORD_OF_NIGHT_EFFECT.get())) return;

        // メインハンドにSword of Nightを持っているかチェック
        if (player.getMainHandItem().getItem() != TheFourPrimitivesAndWeaponsModItems.SWORD_OF_NIGHT.get()) return;
        ...
        // エフェクトを解除（1回限り）
        player.removeEffect(TheFourPrimitivesAndWeaponsModMobEffects.SWORD_OF_NIGHT_EFFECT.get());
