...
### 属性レベルによるダメージ底上げ（全属性共通）
属性ごとのハンドラは、氷や魂のようにレベルで倍率が伸びるものと、炎/水/侵食/闇/瘴気/電気のように
倍率が固定のものが混在している。そのため命中時に「属性が足した分」へ共通のスケールを掛ける
(`ElementalDamageUtils.scaleElementBonusByLevel`、通常攻撃の経路 `ElementalDamageEvent` で適用)。

- 増幅: 属性が足した分 × (1 + 3%/レベル)、上限 +60%
- 最低保証: 素ダメージ × 6%/レベル、上限 素ダメージの150%
- 上記の**大きい方**を採用。Lv1では従来と完全に同じ
- 属性側が意図的にダメージを下げている場合（血の対アンデッド0.9x等）は底上げしない

例: 素ダメージ10 / Lv12 → 瘴気は 11.0 → 16.6、氷は 45.0 → 56.5
